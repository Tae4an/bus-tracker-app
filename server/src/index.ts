...
// 서버 포트 설정 (환경 변수 또는 기본값 8080)
const PORT = process.env.PORT || 8080;

// HTTP 서버 시작
server.listen(Number(PORT), '0.0.0.0', () => {
  const address = server.address() as AddressInfo;
  logger.info(`서버가 ${process.env.NODE_ENV} 모드로 ${address.port} 포트에서 실행 중입니다`);
  logger.info(`http://localhost:${address.port} 에서 API에 접근할 수 있습니다`);
});
